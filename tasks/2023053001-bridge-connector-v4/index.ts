...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  logger.info(`Checking Bridge Connector v3...`)
  const registry = await task.inputDeployedInstance('Registry')
  const bridgeConnectorV3Data = await registry.implementationData(input.BridgeConnectorV3)
  if (!bridgeConnectorV3Data.deprecated) {
    logger.info(`Deprecating Bridge Connector v3...`)
    const admin = await getSigner(input.admin)
    await registry.connect(admin).deprecate(input.BridgeConnectorV3)
  } else {
    logger.warn(`Bridge Connector v3 already deprecated`)
  }
}
