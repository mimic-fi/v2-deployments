...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  const bridger = await grantAdminPermissions(task, 'L2HopBridger', manager, owner)
  const swapper = await grantAdminPermissions(task, 'L2HopSwapper', manager, owner)
  ...
  const relayerFunder = await deployCreate3(
    ...
    { ...create3Params, instanceName: 'RelayerFunder' }
  )
