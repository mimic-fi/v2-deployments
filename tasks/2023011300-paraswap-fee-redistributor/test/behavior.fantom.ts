...
export function itDeploysParaswapFeeRedistributorCorrectly(): void {
  ...
  describe('erc20 claimer', () => {
    ...
    it('sets the expected gas limits', async () => {
      ...
      expect(await erc20Claimer.payingGasToken()).to.be.equal(WFTM)
    })
  ...
  describe('native claimer', () => {
    ...
    it('sets the expected gas limits', async () => {
      ...
      expect(await nativeClaimer.payingGasToken()).to.be.equal(WFTM)
    })
  ...
  describe('swap fee setter', () => {
    ...
    it('sets the expected gas limits', async () => {
      expect(await swapFeeSetter.gasPriceLimit()).to.be.equal(300e9)
      expect(await swapFeeSetter.totalCostLimit()).to.be.equal(0)
      expect(await swapFeeSetter.payingGasToken()).to.be.equal(WFTM)
    })
