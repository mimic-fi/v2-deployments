...
const CHAINLINK_USDC_USD = '0xF096872672F44d6EBA71458D74fe67F9a77a23B9'

const FEE_CLAIMER = '0xbFcd68FD74B4B458961495F3392Bf96f46A29E67'
...
export function itDeploysParaswapFeeRedistributorCorrectly(): void {
  ...
  describe('smart vault', () => {
    ...
    it('sets the expected price feeds', async function () {
      ...
      expect(await smartVault.getPriceFeed(WAVAX, USD)).to.be.equal(CHAINLINK_AVAX_USD)
    })
  })
  ...
  describe('erc20 claimer', () => {
    it('has set its permissions correctly', async () => {
      await assertPermissions(erc20Claimer, [
        ...
        { name: 'mimic', account: mimicAdmin, roles: ['authorize', 'unauthorize', 'setPermissiveMode'] },
    ...
    it('sets the expected gas limits', async () => {
      expect(await erc20Claimer.gasPriceLimit()).to.be.equal(50e9)
      expect(await erc20Claimer.totalCostLimit()).to.be.equal(0)
      expect(await erc20Claimer.payingGasToken()).to.be.equal(WAVAX)
    })
  ...
  describe('native claimer', () => {
    ...
    it('sets the expected gas limits', async () => {
      expect(await nativeClaimer.gasPriceLimit()).to.be.equal(50e9)
      expect(await nativeClaimer.totalCostLimit()).to.be.equal(0)
      expect(await nativeClaimer.payingGasToken()).to.be.equal(WAVAX)
    })
  ...
  describe('swap fee setter', () => {
    ...
    it('sets the expected gas limits', async () => {
      expect(await swapFeeSetter.gasPriceLimit()).to.be.equal(50e9)
      expect(await swapFeeSetter.totalCostLimit()).to.be.equal(0)
      expect(await swapFeeSetter.payingGasToken()).to.be.equal(WAVAX)
    })
