...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  ...
  const smartVaultDeployer = await deployCreate3(task, 'L2SmartVaultDeployer', [], {
    ...create3Params,
    instanceName: 'SmartVaultDeployer',
  })

  const actionParams = [smartVaultDeployer.address, Registry]
  const bridger = await deployCreate3(task, 'L2HopBridger', actionParams, create3Params)
  const swapper = await deployCreate3(task, 'L2HopSwapper', actionParams, create3Params)
