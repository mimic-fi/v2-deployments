...
export default function itDeploysMimicFeeCollectorConnextBridgerCorrectly(): void {
  let input: MimicFeeCollectorConnextBridgerDeployment
  ...
  before('load accounts', async function () {
    input = this.task.input() as MimicFeeCollectorConnextBridgerDeployment
  ...
  before('load instances', async function () {
    ...
    bridger = await this.task.deployedInstance('ConnextBridger')
  })
  ...
  describe('bridger', () => {
    ...
    it('sets the expected max relayer fee pct', async () => {
      expect(await bridger.maxRelayerFeePct()).to.be.equal(fp(0.1))
    })
  })
}
