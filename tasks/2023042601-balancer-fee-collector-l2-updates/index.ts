...
export default async (task: Task, { force, from }: TaskRunOptions = {}): Promise<void> => {
  const input = task.input() as BalancerFeeCollectorL2Updates
  ...
  const bridger = await task.inputDeployedInstance('L2HopBridger')
  ...
  await executePermissionChanges(
    ...
    [
      ...
      {
        target: bridger, // grant owner to set bridger amms
        changes: [{ grant: true, permission: { who: owner, what: 'setTokenAmm' } }],
      },
    ],
    owner
  )
  ...
  // Disable bridger options except for the threshold token
  const tokens = await bridger.getTokens()
  for (const token of tokens) {
    if (token != thresholdToken) {
      await bridger.connect(owner).setTokenAmm(token, ZERO_ADDRESS)
    }
  }
}
