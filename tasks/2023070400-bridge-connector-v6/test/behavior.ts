...
export default function itDeploysBridgeConnectorCorrectly(): void {
  ...
  it('has the expected address', async function () {
    expect(bridgeConnector.address).to.be.equal('0xAc39ff12d96E264eC64d74D0778c167E298Ba0b1')
  })
  ...
  it('deprecates the previous bridge connector', async function () {
    ...
    const { BridgeConnectorV5 } = this.task.input() as BridgeConnectorDeployment
    const data = await registry.implementationData(BridgeConnectorV5)
