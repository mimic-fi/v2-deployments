...
export default function itDeploysBridgeConnectorCorrectly(): void {
  ...
  it('has the expected address', async function () {
    expect(bridgeConnector.address).to.be.equal('0x5F68583cF764c914a7Ca52f224ee9Ffb10A89850')
  })
  ...
  it('deprecates the previous bridge connector', async function () {
    ...
    const { BridgeConnectorV2 } = this.task.input() as BridgeConnectorDeployment
    const data = await registry.implementationData(BridgeConnectorV2)
