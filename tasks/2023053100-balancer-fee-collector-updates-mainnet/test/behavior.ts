...
const AURAL_BAL = '0x616e8bfa43f920657b3497dbf40d6b1a02d4608d'

export default function itUpdatesBalancerFeeCollectorMainnetCorrectly(): void {
  let input: BalancerFeeCollectorUpdatesMainnet
  ...
  before('load input', async function () {
    input = this.task.input() as BalancerFeeCollectorUpdatesMainnet
  })
  ...
  describe('smart vault', () => {
    ...
    it('unsets old actions permissions', async () => {
      await assertPermissions(smartVault, [
        ...
        { name: 'old withdrawer', account: input.Withdrawer, roles: [] },
      ])
    })
  })
  ...
  describe('claimer', () => {
    ...
    it('sets the expected relayer config', async () => {
      ...
      expect(gasLimits.gasPriceLimit).to.be.equal(100e9)
    ...
    it('sets the expected token threshold params', async () => {
      ...
      expect(threshold.min).to.be.equal(toUSDC(5000))
  ...
  describe('1inch swapper', () => {
    ...
    it('sets the expected swapper config', async () => {
      ...
      const customTokenOut = await oneInchSwapper.getCustomTokenOut(AURAL_BAL)
      expect(customTokenOut[0]).to.be.true
      expect(customTokenOut[1]).to.be.equal(BAL)
    })
  ...
  describe('paraswap swapper', () => {
    ...
    it('sets the expected swapper config', async () => {
      ...
      const customTokenOut = await paraswapSwapper.getCustomTokenOut(AURAL_BAL)
      expect(customTokenOut[0]).to.be.true
      expect(customTokenOut[1]).to.be.equal(BAL)
    })
